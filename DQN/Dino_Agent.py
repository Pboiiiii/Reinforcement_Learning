        ...
        def running(self, is_training=True):
            ...
            if is_training:
                ...
                if os.path.exists(os.path.join(f'D:\Python weas\RL\DQN_w_pytorch\{RUNS_DIR}', f'{self.hyperparameter_set}.pt')):
                    log_message = (f"{datetime.now().strftime(DATE_FORMAT)}: Model file, {self.hyperparameter_set}.pt, found\n"
                                   f"{datetime.now().strftime(DATE_FORMAT)}: Loading Model file: {self.hyperparameter_set}.pt")
                    print(log_message)
                    with open(self.LOG_FILE, 'a') as file:
                        file.write(log_message + '\n')
                    ...
                    log_message = (f"{datetime.now().strftime(DATE_FORMAT)}: No model file found\n"
                                   f"{datetime.now().strftime(DATE_FORMAT)}: Starting from scratch")
                    print(log_message)
                    with open(self.LOG_FILE, 'a') as file:
                        file.write(log_message + '\n')
